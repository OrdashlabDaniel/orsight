...
export default function FormsPoolBoard() {
  ...
  async function handleCloneForm(form: FormDefinition) {
    ...
    try {
      ...
      setNoticeMessage(cloned ? `已克隆出新填表：${cloned.name}` : "填表已克隆。");
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "克隆填表失败。");
    } finally {
      setWorkingKey("");
    }
  }
      ...
      <div className="mx-auto max-w-6xl space-y-4">
        <header className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-600">
            在这里管理多个填表。每个填表都有独立的填表模式、训练模式、训练池和工作规则。
          </p>
          {noticeMessage ? (
        ...
        <section className="grid gap-4 md:grid-cols-2">
          ...
          <article className="rounded-2xl border border-dashed border-slate-300 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">
              创建新的独立填表空间。新填表会从完全空白开始，不会带入旧模板和旧表格项目。
            </p>
            <div className="mt-5">
              <button
                type="button"
