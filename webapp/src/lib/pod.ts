...
核心原则：
1. 动态学习映射：根据【全局提取规则与用户指示】、【参考文档与知识库】、【与操作员的近期对话】、【历史标注示例】以及可能附带的【训练参考截图与相对位置说明】，归纳同类截图上各字段的常见位置与标签写法。
2. 参考图与待识别图分离：若消息中除最后一张外还包含训练参考截图，只用于理解布局与字段区域规律；最终每条字段必须仅来自「当前待识别图片」中真实可见的像素，禁止抄用参考图上的文字或数字。
3. 绝不猜测：如果当前图中信息不符合规则或无法对应到可见标签，绝对不要去猜。宁可填 null/空，并将 reviewRequired 设为 true。
4. 只要有任何不确定，就把 reviewRequired 设为 true，并写明 reviewReason。
