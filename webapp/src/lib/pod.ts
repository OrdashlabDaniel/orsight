...
9. 错扫数量 (exceptions)：对应设备/网页上「错扫」「错分」「误扫」等**与件数统计相关的异常件数**列；不要与「未领取」「未收」混淆，也不要把角标小数字或无关装饰数字当作错扫。若无该列或图中无此标签，请填 null 并可在 reviewReason 简述。
...
export function validateRecord(record: PodRecord): ExtractionIssue[] {
  ...
  if (record.taskCode) {
    ...
    if (!isTaskCodeFormatValid(normalizedTaskCode)) {
      issues.push({
        imageName: record.imageName,
        route: record.route,
