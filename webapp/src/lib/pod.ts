...
5. 若存在多行数据（例如网页表格、手写表格），请提取**所有**清晰可见的行，将每一行作为一个独立的记录放入 records 数组中。
6. 响应更新状态 (waybillStatus) 的逻辑：如果未收数量为 0，则状态为 "全领取"；如果未收数量大于 0，则状态为 "待更新"。绝对不要使用 "正常" 等其他词汇。
7. 错扫数量 (exceptions)：如果表格中没有明确的错扫数量列，请填 null。
...
export function organizeRecords(records: PodRecord[]): OrganizedRecordsResult {
  ...
  for (const record of records) {
    ...
    existing.waybillStatus = existing.waybillStatus || record.waybillStatus;
  }
